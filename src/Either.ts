...
export interface Either<L, R> extends Monad<R>, EqualityComparable<Either<L, R>> {
    ...
    getOrQueriedValueNotPresent(msg?: string): Either<QueriedValueNotPresent, R>;
    get(): L|R;
...
export class Left<L, R> implements Either<L, R> {
    ...
    getOrQueriedValueNotPresent(msg?: string): Either<QueriedValueNotPresent, R> {
        return new Left<QueriedValueNotPresent, R>(new QueriedValueNotPresent(msg ?? 'Either::getOrQueriedValueNotPresent'));
    }
...
export class Right<L, R> implements Either<L, R> {
    ...
    zip<L2, R2>(other: Either<L2, R2>): Either<L | L2, [R, R2]> {
        const otherVal: L2|R2 = other.get();
        return other.isLeft() ?
            new Left<L2, [R, R2]>(otherVal as L2) :
            new Right<L | L2, [R, R2]>([this.value, otherVal as R2]);
    }
    ...
    zip6<L2, L3, L4, L5, L6, L7, R2, R3, R4, R5, R6, R7>(
        ...
        return o1 instanceof Left
            ? new Left<L | L2 | L3 | L4 | L5 | L6 | L7, [R, R2, R3, R4, R5, R6, R7]>(
                o1.get()
            )
            : o2 instanceof Left
                ? new Left<L | L2 | L3 | L4 | L5 | L6 | L7, [R, R2, R3, R4, R5, R6, R7]>(
