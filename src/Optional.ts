...
export interface Optional<T> extends Monad<T>, EqualityComparable<Optional<T>> {
    ...
    zipN<U>(others: Optional<U>[]): Optional<[T, ...U[]]>;
    getOrElse(x: T): T;
...
export class None<T> implements Optional<T> {
    ...
    getOrQueriedValueNotPresent(msg?: string): Either<QueriedValueNotPresent, T> {
        return new Left<QueriedValueNotPresent, T>(new QueriedValueNotPresent(msg ?? `Optional::getOrQueriedValueNotPresent`));
    }
...
export function optionalFromValue<T>(t: T): Optional<T> {
